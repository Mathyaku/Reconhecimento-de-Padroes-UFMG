...
svm.model <- svm(D ~ ., data = X[,1:2], cost=1000, gamma = 0.01)


alphas <- svm.model$coefs
alphasI <- svm.model$index


## alpha referente ao vetores de suporte
par(new=T)
plot(X[alphasI,1],X[alphasI,2],xlim=c(-3,8),ylim=c(-3,8), col="green")
